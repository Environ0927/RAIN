...
The primary paper matrix is locked to FEMNIST, CIFAR-10, and Tiny-ImageNet.
CIFAR-100 remains a supported auxiliary extension and is not included in the
primary tables unless the paper scope is explicitly changed.
...
| Tiny-ImageNet | small-image ResNet-18 | 200 | 11,271,432 | 3x64x64 |
...
python -m rain.cli.train --config configs/quick/cifar10_resnet18.json \
  --output outputs/cifar10-r18-quick --device cuda

python -m rain.cli.train --config configs/quick/tinyimagenet_resnet18.json \
  --output outputs/tiny-r18-quick --device cuda
```
...
python -m rain.cli.calibrate_large \
  --config configs/large/cifar10_resnet18.json --device cuda

python -m rain.cli.calibrate_large \
  --config configs/large/tinyimagenet_resnet18.json --device cuda
```
...
python -m rain.cli.train --config configs/large/cifar10_resnet18.json \
  --output outputs/cifar10-r18-full --device cuda

python -m rain.cli.train --config configs/large/tinyimagenet_resnet18.json \
  --output outputs/tiny-r18-full --device cuda
```
...
python -m rain.cli.make_sweep \
  ...
  --attacks none,krum,min-max,scaling,attack-dpfl,raa,woaa \
  --ratios 0.1,0.2,0.4 --hamming-fraction 0.25

python -m rain.cli.make_sweep \
  --base configs/large/cifar10_resnet18.json \
  --output-dir configs/generated/cifar10-r18 \
  --seeds 1,2,3 \
  --aggregations rain,foundationfl,fltrust,flod,rflpa \
  --attacks none,krum,min-max,scaling,attack-dpfl,raa,woaa \
  --ratios 0.1,0.2,0.4 --hamming-fraction 0.25
...
python -m rain.cli.make_sweep \
  --base configs/large/tinyimagenet_resnet18.json \
  --output-dir configs/generated/tinyimagenet-r18 \
  --seeds 1,2,3 \
  --aggregations rain,foundationfl,fltrust,flod,rflpa \
  --attacks none,krum,min-max,scaling,attack-dpfl,raa,woaa \
  --ratios 0.1,0.2,0.4 --hamming-fraction 0.25
...
The generated manifest contains the exact command for every run. RAA's budget
is converted from the public fraction to an exact model-coordinate count and
remains below the filtering threshold. WOAA instead searches every radius up
to `floor(d*tau)`, as specified in the current paper.
...
python -m rain.cli.large_benchmark \
  --clients 2,5,10,20 \
  --dimensions 3246270,11173962,11271432 \
...
2. Scaling ASR, Attack-DPFL, RAA, and WOAA at 10%, 20%, and 40% malicious clients;
