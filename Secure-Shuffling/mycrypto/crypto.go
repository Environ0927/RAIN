...
// return a message
func MakeMsg(numBlocks, msgType int) []byte { //numBlocks = 2
	blockSize := 16
...
func GenBeavers_single(numBeavers, seedIndex int, seeds [][]byte) [][]byte {
	...
	//expand a and b shares
	for i := 0; i < numServers; i++ {
		beaversA[i] = AesPRG(16*numBeavers, seeds[i][seedIndex:seedIndex+16])
		beaversB[i] = AesPRG(16*numBeavers, seeds[i][seedIndex+16:seedIndex+32])
	}
...
func CheckSharesAreZero(batchSize, numServers int, shares []byte) bool {
	...
	numThreads, chunkSize := PickNumThreads(batchSize) // numThreads := 16, chunkSize=batchSize/16
	res := true
	resChan := make(chan bool)
		...
		go func(startI, endI int) {
			...
			for i := startI; i < endI; i++ {
				...
				if !hopefullyZero.IsZero() {
					innerRes = false
					// log.Printf("MAC check in individual batch", innerRes)
				} else {
					innerRes = true
					// log.Printf("MAC check in individual batch", innerRes)
				}
...
func CheckSharesAreZero_singleThread(batchSize, numServers int, shares []byte) bool {
...
func BeaverProduct_singleThread(msgBlocks, batchSize int, beaversC, mergedMaskedShares []byte, db [][]byte, leader, aggregate bool) []byte {
	// mergedMaskedShares = (maskedExpandedKeyShares, maskedMsgShares)
	keyBlocks := msgBlocks
	...
	for i := 0; i < batchSize; i++ {
		var maskedKey, myKeyShare, maskedMsg, myMsgShare, givenTag, temp modp.Element
		var runningSum, beaverProductShare modp.Element
		for j := 0; j < keyBlocks; j++ { // 每一个block进行 Beaver乘法
			//do a beaver multiplication here
			keyShareIndex := 16*(msgBlocks+1) + 16*j
			myKeyShare.SetBytes(db[i][keyShareIndex : keyShareIndex+16])
			myMsgShare.SetBytes(db[i][16*j : 16*(j+1)])
...
func GetMaskedStuff(batchSize, msgBlocks int, beaversA, beaversB []byte, db [][]byte, partTwo bool) []byte {
...
// ======== Rainy: Hamming Distance in Plaintext (for integration) ========

// HammingPlain: 明文模式下计算每条消息的 Hamming 距离
// rows: 每行长度必须为 16*(k+1)，前 k 个 16B 为比特(0/1)，最后 16B 为 seed
// sTrustBits16: 长度 16*D，每位 0/1，用 16B 表示
func HammingPlain(rows [][]byte, D, k int, sTrustBits16 []byte) ([]int, error) {
	if len(sTrustBits16) != 16*D {
		return nil, fmt.Errorf("s_trust length must be 16*D")
	}
	hd := make([]int, len(rows))
	for i, row := range rows {
		if len(row) != 16*(k+1) {
			return nil, fmt.Errorf("row length must be 16*(k+1)")
		}
		seed := row[16*k : 16*(k+1)]
		_ = seed // 先不用 seed; 后续 deriveIndex 替换成随机索引
		sum := 0
		for j := 0; j < k; j++ {
			b := row[16*j]
			s := sTrustBits16[16*j] // 暂取前 k 位对应 s_trust；正式版需 deriveIndex
			if (b^s)&1 == 1 {
				sum++
			}
		}
...
// ======== Rainy: Hamming Distance under MPC (using Beaver triples) ========

func HammingShares(
    dbRows [][]byte,   // 每行 16*(k+1)：前 k 比特 + seed16
    D, k int,
    sTrustShare []byte, // 本方持有的 s_trust 份额 (16*D)
    beaverSeeds [][]byte, seedIndex int, // 每方的 Beaver 种子（96B/方）
    myServerNum int, leader bool,
    send func([]byte) error, recv func(int) ([]byte, error),
) ([]byte, error) {

    if len(sTrustShare) != 16*D {
        return nil, fmt.Errorf("s_trust share length must be 16*D")
    }
    L := len(dbRows) * k
    ...
    for _, row := range dbRows {
        seed := row[16*k : 16*(k+1)]
        for j := 0; j < k; j++ {
            idx := deriveIndex(seed, j, D)
            bij := row[16*j : 16*(j+1)]
            sij := sTrustShare[16*idx : 16*(idx+1)]
            bcopy, scopy := make([]byte, 16), make([]byte, 16)
            copy(bcopy, bij)
            copy(scopy, sij)
            bVec = append(bVec, bcopy)
            sVec = append(sVec, scopy)
        }
    }

    // 生成 Beaver 三元组
    aMask := AesPRG(16*L, beaverSeeds[myServerNum][seedIndex:seedIndex+16])
    bMask := AesPRG(16*L, beaverSeeds[myServerNum][seedIndex+16:seedIndex+32])
    cShares := GenBeavers(L, seedIndex, beaverSeeds)
    cShare := cShares[myServerNum]

    // 计算 masked 向量
    maskedB := make([]byte, 16*L)
    maskedS := make([]byte, 16*L)
    copy(maskedB, flatten(bVec))
    copy(maskedS, flatten(sVec))
    AddOrSub(maskedB, aMask, false)
    AddOrSub(maskedS, bMask, false)

    // 交换 masked 向量
    if err := send(maskedB); err != nil { return nil, err }
    if err := send(maskedS); err != nil { return nil, err }
    otherMaskedB, err := recv(16*L); if err != nil { return nil, err }
    otherMaskedS, err := recv(16*L); if err != nil { return nil, err }

    maskedBsum := make([]byte, 16*L)
    maskedSsum := make([]byte, 16*L)
    copy(maskedBsum, maskedB)
    copy(maskedSsum, maskedS)
    AddOrSub(maskedBsum, otherMaskedB, true)
    AddOrSub(maskedSsum, otherMaskedS, true)

    // Beaver 乘法 (share of b*s)
    prodVec := make([][]byte, L)
    for t := 0; t < L; t++ {
        var mb, ms, ai, bi, ci, prod, tmp modp.Element
        mb.SetBytes(maskedBsum[16*t : 16*(t+1)])
        ms.SetBytes(maskedSsum[16*t : 16*(t+1)])
        ai.SetBytes(aMask[16*t : 16*(t+1)])
        bi.SetBytes(bMask[16*t : 16*(t+1)])
        ci.SetBytes(cShare[16*t : 16*(t+1)])
        prod.SetZero()
        if leader { prod.Mul(&mb, &ms) }
        tmp.Mul(&mb, &bi)
        prod.Add(&prod, &tmp)
        tmp.Mul(&ms, &ai)
        prod.Add(&prod, &tmp)
    ...
    // 每条消息汇总 xor = b + s − 2*b*s
    hdShares := make([]byte, 16*len(dbRows))
    var two modp.Element
    two.SetUint64(2)
    rowBase := 0
    for i := 0; i < len(dbRows); i++ {
        var sum, tmp, two_bs, bij, sij, bs modp.Element
        for j := 0; j < k; j++ {
            pos := rowBase + j
            bij.SetBytes(bVec[pos])
            sij.SetBytes(sVec[pos])
            bs.SetBytes(prodVec[pos])
            tmp.Add(&bij, &sij)
            two_bs.Mul(&two, &bs)
            tmp.Sub(&tmp, &two_bs)
            sum.Add(&sum, &tmp)
        }
        copy(hdShares[16*i:16*(i+1)], sum.Bytes())
        rowBase += k
    }
    return hdShares, nil
}
...
// ======== Rainy: Hamming Distance under simulated MPC (no network) ========
func HammingSharesSim(dbRows [][]byte, D, k int, sTrustShare []byte) ([]byte, error) {
	if len(sTrustShare) != 16*D {
		return nil, fmt.Errorf("s_trust share length must be 16*D")
	}
	L := len(dbRows) * k
	...
	// 2) Beaver triple 模拟：直接用明文乘法代替
	prodVec := make([][]byte, L)
	for i := 0; i < L; i++ {
		var b, s, prod modp.Element
		b.SetBytes(bVec[i])
		s.SetBytes(sVec[i])
		prod.Mul(&b, &s)
		prodBytes := make([]byte, 16)
		copy(prodBytes, prod.Bytes())
		prodVec[i] = prodBytes
	}

	// 3) per-row 汇总：xor = b + s − 2*b*s
	hdShares := make([]byte, 16*len(dbRows))
	var two modp.Element
	two.SetUint64(2)
	rowBase := 0
...
// Rainy: single-node simulation of Camel secure shuffle
func SimulateSecureShuffle(flatDB []byte, batchSize, blocksPerRow int) ([]byte, []int, error) {
    numServers := 2
    seeds := make([][]byte, numServers)
    for i := 0; i < numServers; i++ {
        seeds[i] = make([]byte, 96)
        _, err := rand.Read(seeds[i])
        if err != nil {
            return nil, nil, fmt.Errorf("rand.Read: %v", err)
        }
    }

    delta := GenShareTrans(batchSize, blocksPerRow, seeds)
    perms := GenPerm(batchSize, seeds[0][80:96]) // 第一台服务器的置换

    shuffled := PermuteDB(flatDB, perms)
    AddOrSub(shuffled, delta, true) // 模拟 mask
    return shuffled, perms, nil
}


func flatten(chunks [][]byte) []byte {
    out := make([]byte, 16*len(chunks))
    for i := 0; i < len(chunks); i++ {
        copy(out[16*i:16*(i+1)], chunks[i])
    }
    return out
}
