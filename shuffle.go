...
func exp_camel(vec_dim, batchsz int) {
	...
	// === HAMMING START ===
	// 已知：
	//   - db0Rows, db1Rows: 两个服务器的混洗后“行式数据库”（[][]byte）
	//   - batchSize, msgBlocks, blocksPerRow：行大小参数
	//   - vecDimD: 可信方向 s_trust 的维度（= 模型总维度 D）
	//   - k = msgBlocks - 1
	//   - tauCount: 公开阈值（以“计数”单位；若你在 Python 端用比例 τ∈[0,1]，请用 ceil(τ*k) 转换成计数）
	//
	// 你需要把 sTrust 的按位份额传进来（16*D 字节）；
	// 方式一：从文件载入
	//    sTrustShare0 := ioutil.ReadFile("strust_share_party0.bin")
	//    sTrustShare1 := ioutil.ReadFile("strust_share_party1.bin")
	// 方式二：从网络/上层函数参数传入（推荐）
	//
	// 下面假设我们当前是“party 0”，持有 sTrustShare0、db0Rows；party 1 类似。
	//
	k := msgBlocks - 1
	
	// 计算本方的 hd 份额
	hdShares0, err := mycrypto.HammingShares(
	    db0Rows, batchSize, msgBlocks, k, blocksPerRow, vecDimD, sTrustShare0,
	)
	if err != nil {
	    panic(err)
	}
	
	// 交换并合并分享，得到明文 hd（MVP：明文比较；如需密文比较可继续走电路）
	// TODO: 用你现有的“合并加法分享”的函数；这里占位为 MergeShares(hdShares0, hdShares1)
	hdShares1 := recvFromOtherParty()              // ← 你现有的通信函数
	hdMerged := MergeShares(hdShares0, hdShares1)  // ← 把两方 hd 份额按 mod p 相加得到明文
	
	// 明文比较：得到 pass_mask
	passMask := make([]bool, batchSize)
	for i := 0; i < batchSize; i++ {
	    // 将 16B modp.Element 解析为 uint64（或 int）
	    var hd_i mycrypto.Element
	    hd_i.SetBytes(hdMerged[16*i : 16*(i+1)])
	    val := hd_i.Uint64() // 若你没有 Uint64()，可以把 Bytes() 再转
	    passMask[i] = (val <= uint64(tauCount))
	}
	
	// 将 passMask 返回给上层（例如通过 socket 回 Python），用于 TSSC-ReLU 的后续 ReLU 加权与投票
	sendToUpperLayer(passMask)
	 // === HAMMING END ===
