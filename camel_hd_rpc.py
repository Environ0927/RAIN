...
def call_camel_hd(bin_path: str, D: int, k: int, tau_count: int,
    ...
    if "pass_mask" not in out:
        raise RuntimeError(f"camel_hd failed: {p.stderr.decode()} {out}")
    return out["pass_mask"], out.get("hd")

def call_camel_hd(bin_path, D, k, tau_count, rows, s_trust_bits16, mode="plain"):
    req = {
    ...
    if "pass_mask" not in out and "hd" not in out and "msg" in out:
        raise RuntimeError(f"camel_hd error: {out['msg']}")
    return out.get("pass_mask"), out.get("hd")

def call_camel_shuffle(bin_path, flat_bytes: bytes, D: int, k: int):
    req = {
        "D": D,
        "k": k,
        "mode": "shuffle",
...
def call_camel_shuffle_perm(bin_path: str, n_rows: int, blocks_per_row: int, round_id: int):
    req = {
        "D": n_rows,
        "k": blocks_per_row,
        "mode": "shuffle",
        "round_id": round_id,                # ← 新增
        "rows_b64": [],                      # 只取 perm 时可为空
    }
    p = subprocess.run([bin_path], input=json.dumps(req).encode(),
                       stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out = json.loads(p.stdout.decode() or "{}")
    if "perm" not in out:
        raise RuntimeError(f"camel_hd shuffle error: {out.get('msg') or p.stderr.decode()}")
    return out["perm"]