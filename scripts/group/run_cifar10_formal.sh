...
run_method() {
    ...
    for seed in 1 2 3; do
        local output="$output_root/cifar10-${method}-seed${seed}"
        local raw="$output/rounds.jsonl"
        local rows=0
        if [[ -f "$raw" ]]; then rows="$(wc -l < "$raw")"; fi
        if (( rows >= 1000 )); then
            echo "[$(date -Is)] skipping completed $method seed=$seed"
            continue
        fi
        local resume_args=()
        ...
        echo "[$(date -Is)] starting formal $method seed=$seed lr=$learning_rate on CUDA index $gpu"
        CUDA_VISIBLE_DEVICES="$gpu" "$python_bin" -u -m rain.cli.train \
            ...
            >> "$output_root/cifar10-${method}-seed${seed}.log" 2>&1
    done
}
